...
describe('OpenPlugin End to End Tests', () => {
  ...
  test('yt_caption_retriever', async () => {
    const plugin = new OpenPlugin("yt_caption_retriever", OPENAI_API_KEY, undefined, true);
    await plugin.init();
    expect(plugin.manifest).not.toBeNull();
  
    const chatgpt_prompt = 'give me a 2 sentence summary of the following yt video https://www.youtube.com/watch?v=P310I19L3Ko';
    const response = await plugin.fetch_plugin({
      prompt: chatgpt_prompt,
      model: "gpt-3.5-turbo-0613",
      temperature: 0,
  ...
  test('twtData', async () => {
    ...
    const chatgpt_prompt = 'show me the amount of people @Sebasti54919704 is following';
    const response = await plugin.fetch_plugin({
      prompt: chatgpt_prompt,
      model: "gpt-3.5-turbo-0613",
      temperature: 0,
  ...
  test('tailor_erp', async () => {
    const plugin = new OpenPlugin("tailor_erp", OPENAI_API_KEY, undefined, true);
    await plugin.init();
    expect(plugin.manifest).not.toBeNull();
  ...
  test('surge_ai_trends', async () => {
    const plugin = new OpenPlugin("surge_ai_trends", OPENAI_API_KEY, undefined, true);
    await plugin.init();
    expect(plugin.manifest).not.toBeNull();
  ...
  test('speedy_marketing', async () => {
    const plugin = new OpenPlugin("speedy_marketing", OPENAI_API_KEY, undefined, true);
    await plugin.init();
    expect(plugin.manifest).not.toBeNull();
  ...
  test('scholarai', async () => {
    ...
    const chatgpt_prompt = 'What scientific research exists for semantic representation of language through brain waves. dont sort.';
    const response = await plugin.fetch_plugin({
      prompt: chatgpt_prompt,
      model: "gpt-3.5-turbo-0613",
      temperature: 0,
  ...
  test('rephrase', async () => {
    ...
    const chatgpt_prompt = 'I want to code a react ui with hello world please rephrase that';
    const response = await plugin.fetch_plugin({
      prompt: chatgpt_prompt,
      model: "gpt-3.5-turbo-0613",
      temperature: 0,
  ...
  test('DreamInterpreter', async () => {
    const plugin = new OpenPlugin("DreamInterpreter", OPENAI_API_KEY, undefined, true); // Adjust the url as necessary
    await plugin.init();
    expect(plugin.manifest).not.toBeNull();
    
    const chatgpt_prompt = 'I dreamt of being in a room without any windows getting smaller overtime';
    const response = await plugin.fetch_plugin({
      prompt: chatgpt_prompt,
      model: "gpt-3.5-turbo-0613",
      temperature: 0,
  ...
  test.skip('portfoliopilot', async () => { // 400 status code when pinging api, probably misformatted args
    const plugin = new OpenPlugin("portfoliopilot", OPENAI_API_KEY, undefined, true);
    await plugin.init();
    expect(plugin.manifest).not.toBeNull();
  
    const chatgpt_prompt = 'What stocks should I add for my long term tech portfolio';
    const response = await plugin.fetch_plugin({
      prompt: chatgpt_prompt,
      model: "gpt-3.5-turbo-0613",
      temperature: 0,
  ...
  test.skip('C3_Glide', async () => { // 400 status code when pinging api, probably misformatted args
    const plugin = new OpenPlugin("C3_Glide", OPENAI_API_KEY, undefined, true);
    await plugin.init();
    expect(plugin.manifest).not.toBeNull();
  
    const chatgpt_prompt = 'Provide me TAF for KJFK with reguards to aviation weather';
    const response = await plugin.fetch_plugin({
      prompt: chatgpt_prompt,
      model: "gpt-3.5-turbo-0613",
      temperature: 0,
  ...
  test('Ai_PDF', async () => {
    ...
    const chatgpt_prompt = 'Can I have my data be private according to this pdf https://www.unodc.org/pdf/criminal_justice/UN_Basic_Principles_on_the_Role_of_Lawyers.pdf';
    const response = await plugin.fetch_plugin({
      prompt: chatgpt_prompt,
      model: "gpt-3.5-turbo-0613",
      temperature: 0,
